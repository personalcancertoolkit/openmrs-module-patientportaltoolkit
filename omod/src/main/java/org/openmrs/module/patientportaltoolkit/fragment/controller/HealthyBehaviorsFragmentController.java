...
public class HealthyBehaviorsFragmentController {
    ...
    public void controller(@SpringBean("myBean") DataAccessPermissionService permissionService) {
        ...
        pprRelations = Context.getService(PatientPortalRelationService.class)
                .getPatientPortalRelationByPerson(currentPerson);
        for (PatientPortalRelation ppr : pprRelations) {
            if (ppr.getShareStatus() == PatientPortalRelation.SHARE_STATUS_ACCEPTED) {
                ...
                if (Context.getService(
                        PatientPortalRelationService.class)
                        .hasAccessToShareType(currentPerson, otherPerson,
                                Context.getService(SecurityLayerService.class)
                                        .getSecurityLayerByName(PatientPortalToolkitConstants.CAN_SEE_HEALTHYBEHAVIOR),
                ...
                for (PatientPortalRelation ppr : pprRelations) {
                    if (getAccessToPerson.equals(ppr.getPerson()) || getAccessToPerson.equals(ppr.getRelatedPerson())) {
                        ...
                        if (ppr.getShareStatus() == PatientPortalRelation.SHARE_STATUS_ACCEPTED) {
                            ...
                            if (Context.getService(
                                    PatientPortalRelationService.class).hasAccessToShareType(
                                            getAccessToPerson,
                                            getAccessFrom,
                                            Context.getService(SecurityLayerService.class).getSecurityLayerByName(
                                                    PatientPortalToolkitConstants.CAN_SEE_HEALTHYBEHAVIOR),
                                            Context.getAuthenticatedUser())) {
                                hasAccess = true;
                            }
                        }
    ...
    private void checkAndAddPermission(Person person, Person secondPerson,
        ...
        // DataAccessPermissionService permissionService =
        // Context.getService(DataAccessPermissionService.class);
        // check to make sure the permission does not already exist
        System.out.println("Try to find an already existing permission...");
        DataAccessPermission nutritionPermission = permissionService.getDataAccessPermission(person, secondPerson,
                "nutrition_form", "read");
        if (nutritionPermission == null) {
            DataAccessPermission newNutritionPermission = new DataAccessPermission(person, secondPerson,
                    "nutrition_form", "read");
            permissionService.saveDataAccessPermission(newNutritionPermission);
        }
        DataAccessPermission exercisePermission = permissionService.getDataAccessPermission(person, secondPerson,
                "exercise_form", "read");
        if (exercisePermission == null) {
            DataAccessPermission newExercisePermission = new DataAccessPermission(person, secondPerson, "exercise_form",
                    "read");
    ...
    private void checkAndDeletePermission(Person person, Person secondPerson,
        ...
        if (Context.getService(PatientPortalRelationService.class)
                .hasAccessToShareType(person, secondPerson,
                        Context.getService(SecurityLayerService.class)
                                .getSecurityLayerByName(PatientPortalToolkitConstants.CAN_SEE_HEALTHYBEHAVIOR),
                        Context.getAuthenticatedUser())) {
        ...
        DataAccessPermission nutritionPermission2 = permissionService.getDataAccessPermission(secondPerson, person,
                "nutrition_form", "read");
        if (nutritionPermission2 != null) {
            permissionService.deleteDataAccessPermission(nutritionPermission2);
        }
        DataAccessPermission exercisePermission2 = permissionService.getDataAccessPermission(secondPerson, person,
                "exercise_form", "read");
