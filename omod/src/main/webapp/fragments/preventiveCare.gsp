<script>
    ...
    function load_preventive_data() {
        var baseUrl =window.location.href.split("/patientportaltoolkit")[0];
        ...
        var load_all_possible_events = new Promise((resolve, reject)=>{
            ...
            jq.get(baseUrl+'/ws/patientportaltoolkit/getpossiblenewpreventivecareeventsforpatient/'+ jq("#personUuid").val(), function (data) {
                resolve(data);
            });
        })
        var load_all_current_events = new Promise((resolve, reject)=>{
            //Load Reminder data, insert reminders into calendar and table
            jq.get(baseUrl+'/ws/patientportaltoolkit/getpreventivecareforpatient/'+ jq("#personUuid").val(), function (data) {
                resolve(data);
            });
        })
