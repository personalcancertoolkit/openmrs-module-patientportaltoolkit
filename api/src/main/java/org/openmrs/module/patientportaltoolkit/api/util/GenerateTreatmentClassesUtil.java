...
public class GenerateTreatmentClassesUtil {
    ...
    public static List<GeneralHistory> generateGeneralHistory(Patient patient) {
        List<Encounter> encounters = getEncountersByTreatment(patient,
                PatientPortalToolkitConstants.TREATMENTSUMMARY_ENCOUNTER);
    ...
    public static List<Chemotherapy> generateChemotherapies(Patient patient) {
        List<Encounter> encounters = getEncountersByTreatment(patient,
                PatientPortalToolkitConstants.CHEMOTHERAPY_ENCOUNTER);
    ...
    public static List<Radiation> generateRadiations(Patient patient) {
        List<Encounter> encounters = getEncountersByTreatment(patient,
                PatientPortalToolkitConstants.RADIATION_ENCOUNTER);
    ...
    public static GeneralHistory generateLatestGeneralHistory(Patient patient) {
        ...
        for (GeneralHistory gh : genhistoryList) {
            ...
            if (genHistory == null) {
                ...
                Date ghDiagnosisDate = gh.getDiagnosisDate();
                Date genHistoryDiagnosisDate = genHistory.getDiagnosisDate();
                if (ghDiagnosisDate != null && genHistoryDiagnosisDate != null
                        && genHistoryDiagnosisDate.before(ghDiagnosisDate)) {
                    genHistory = gh;
                }
            }
        }
        ...
        for (Encounter encounter : encounters) {
            if (!encounter.getVoided() && treatmentType.equals(encounter.getEncounterType().getName())) {
                if (treatmentEncounter == null)
                    ...
                    Date treatmentEncounterCreatedDate = treatmentEncounter.getDateCreated();
                    Date encounterCreatedDate = encounter.getDateCreated();
                    if (encounterCreatedDate != null && treatmentEncounterCreatedDate != null
                            && treatmentEncounterCreatedDate.before(encounterCreatedDate)) {
                        treatmentEncounter = encounter;
                    }
                }
            }
